...
function normalizeDatabaseUrl(url) {
  ...
  if (url.startsWith('mongodb://') || url.startsWith('mongodb+srv://')) {
    ...
    try {
      ...
      // TLS/SSL explicit settings for MongoDB Atlas (helps with fatal alert errors)
      // Note: mongodb+srv:// uses TLS by default, but explicit params can help
      if (!params.has('tls') && !params.has('ssl')) {
        // For mongodb+srv, TLS is automatic, but we can add it for clarity
        // Don't add tls=false as it breaks mongodb+srv
      }
      
      // Additional parameters that can help with TLS issues in serverless
      if (!params.has('directConnection')) {
        // Don't force direct connection for replica sets (default is false)
      }
