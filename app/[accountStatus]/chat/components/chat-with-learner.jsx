...
export default function ChatWithLearner() {
  ...
  const markAllMessagesAsRead = useCallback(async () => {
    if (selectedLearner?.conversationId && currentUser) {
      try {
        const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/mark-read`, {
          method: 'POST',
          headers: {
  ...
  }, [selectedLearner?.conversationId, currentUser])
  ...
  useEffect(() => {
    const loadMessages = async () => {
      if (selectedLearner && selectedLearner.conversationId) {
        try {
          ...
          if (data.success) {
            ...
            setGroupedMessages(groupMessagesByDate(formattedMessages))
  ...
  useEffect(() => {
    const loadPinnedMessages = async () => {
      if (selectedLearner?.conversationId) {
        try {
          const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/pinned`)
  ...
  useEffect(() => {
    const loadStarredMessages = async () => {
      if (selectedLearner?.conversationId) {
        try {
          const response = await fetch(`/api/users/starred-messages?conversationId=${selectedLearner.conversationId}`)
          const data = await response.json()
  ...
  const handleTyping = () => {
    if (!selectedLearner?.conversationId || !isConnected) return

    if (!isUserTyping) {
      setIsUserTyping(true)
      startTyping(selectedLearner.conversationId)
    }
    ...
    if (typingTimeoutRef.current) {
     ...
     typingTimeoutRef.current = setTimeout(() => {
       ...
       stopTyping(selectedLearner.conversationId)
     }, 5000)
  }
  ...
  const handleReaction = async (messageId, emoji) => {
    if (!selectedLearner?.conversationId || !currentUser) return

    try {
      const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/${messageId}/reactions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handlePinMessage = async (messageId, action) => {
    ...
    try {
      const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/${messageId}/pin`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleStarMessage = async (messageId, action) => {
    ...
    try {
      const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/${messageId}/star`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSearch = async () => {
    if (!messageSearchQuery.trim() || !selectedLearner?.conversationId) return
    ...
    try {
      ...
      const response = await fetch(`/api/conversations/${selectedLearner.conversationId}/messages/search?${params}`)
      const data = await response.json()
  ...
  const updateOnlineStatus = (learnerId, isOnline) => {
    setLearners(prev => prev.map(learner => 
      learner.id === learnerId 
        ? { ...learner, online: isOnline }
        : learner
    ))
  }
  ...
  // Handle delete from me (only sender)
  ...
  // Handle delete from everyone (both sides)
  ...
  const getFilteredLearners = () => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(learner => 
        ...
        learner.industry.toLowerCase().includes(searchQuery.toLowerCase())
      )
    }

    switch (filter) {
      case 'online':
        return filtered.filter(learner => learner.online)
      case 'unread':
        return filtered.filter(learner => learner.unreadCount > 0)
      case 'recent':
        return filtered.filter(learner => learner.lastMessage.includes('minute') || learner.lastMessage.includes('hour'))
      default:
        return filtered
    }
  }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500">{learner.lastMessageTime}</span>
                        {learner.unreadCount > 0 && (
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600 truncate flex-1">{learner.lastMessagePreview}</p>
                      {learner.unreadCount > 0 && (
                        <div className="ml-2">
          ...
          {selectedLearner ? (
            <>
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-300 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={selectedLearner.avatar} alt={selectedLearner.name} />
                    <AvatarFallback className="bg-blue-500 text-white">
                      {selectedLearner.name.split(' ').map(n => n[0]).join('')}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="font-semibold text-gray-900">{selectedLearner.name}</h3>
                     <div className="flex items-center gap-2">
                     <p className="text-xs text-gray-500">
                       {typingUsers.size > 0 ? (
                         ...
                         selectedLearner.online ? 'Online' : 'Last seen recently'
                       )}
                     </p>
                       {typingUsers.size === 0 && (
              ...
              {pinnedMessages.length > 0 && (
                <div className="bg-gradient-to-r from-gray-200 to-gray-300 text-gray-800 px-4 py-2 border-b border-gray-400">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                  ...
                  {showPinnedMessages && (
                    <div className="mt-2 space-y-1 max-h-24 overflow-y-auto">
                      {pinnedMessages.map((pinnedMsg) => (
                        <div
                          key={pinnedMsg.id}
                          className="bg-white/30 backdrop-blur-sm rounded p-2 cursor-pointer hover:bg-white/50 transition-colors"
                          onClick={() => {
                        ...
                        {!msg.isExpert && (
                          <Avatar className="h-8 w-8 flex-shrink-0">
                            ...
                            <AvatarFallback className="bg-blue-500 text-white text-xs">
                              {selectedLearner?.name?.split(' ').map(n => n[0]).join('') || 'L'}
                            </AvatarFallback>
                          </Avatar>
                        )}
            ...
            <div className="flex-1 flex items-center justify-center bg-gray-50">
              <div className="text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">Select a Learner</h3>
                <p className="text-gray-500 max-w-sm">
                  Choose a learner from the list to start a conversation.
                </p>
              </div>
            </div>
          )}
